...
class Index_model extends CI_Model
{
	...
	{
		$pagesize = 10;
		$sqls = "SELECT COUNT(ms.msgid) AS num 
			FROM guwen_message AS ms,guwen_user AS us,guwen_comment AS cmt 
			WHERE ms.msgid = cmt.comment_quesid 
			AND us.user_id = cmt.comment_uid 
			AND ms.user_id != cmt.comment_uid
			AND ms.user_id= ?";
		...
		$sql = "SELECT ms.ques_title,us.user_name,ms.msgid,us.user_id,cmt.comment_time,'$numpage' AS num,cmt.comment_content
			 FROM guwen_message AS ms,guwen_user AS us , guwen_comment AS cmt 
			 WHERE ms.msgid = cmt.comment_quesid AND us.user_id = cmt.comment_uid  
			 AND ms.user_id != cmt.comment_uid AND ms.user_id=?  
	...
	public function get_reply($uid,$pages)
	{
		...
		$sqls = "SELECT COUNT(cmt.id) AS num FROM guwen_comment AS cmt, guwen_comment_reply AS rpy , 
			guwen_user AS us ,guwen_message AS ms 
			WHERE cmt.id = rpy.comment_id AND us.user_id = rpy.reply_uid 
			AND us.user_id != cmt.comment_uid AND cmt.comment_quesid = ms.msgid 
			AND cmt.comment_uid = ?";
		$querys = $this->db->query($sqls,array($uid));
		$re = $querys->result_array();
		$offset = ($pages-1)*$pagesize;
		...
		$sql = "SELECT cmt.comment_content,rpy.reply_content,rpy.time,ms.ques_title,ms.msgid,us.user_name,
			us.user_id,'$numpage' AS num 
			FROM guwen_comment AS cmt, guwen_comment_reply AS rpy , 
			guwen_user AS us ,guwen_message AS ms 
			WHERE cmt.id = rpy.comment_id AND us.user_id = rpy.reply_uid 
			AND cmt.comment_quesid = ms.msgid AND 
			ms.user_id != cmt.comment_uid AND cmt.comment_uid = ? 
	...
	public function get_favour($uid,$pages)
	{
		...
		$sqls = "SELECT  count(slg.id) AS num FROM guwen_user AS us,guwen_message AS ms ,
			guwen_comment as cmt,guwen_snslog AS slg 
			WHERE slg.uid = us.user_id AND slg.is_favour = 0
			AND ms.msgid = slg.quesid  AND slg.comment_id = cmt.id  AND slg.uid != cmt.comment_uid
			AND  cmt.comment_uid = ?";
		$querys = $this->db->query($sqls,array($uid));
		$re = $querys->result_array();
		$offset = ($pages-1)*$pagesize;
	...
	public function get_best($uid,$pages)
	{
		...
		$numpage = ceil($count/$pagesize);
		$sql  = "SELECT us.user_name,us.user_id,ms.ques_title,ms.msgid,cmt.comment_content,blg.time,'$numpage' AS num 
			FROM guwen_user AS us ,guwen_bestanwserlog AS blg,guwen_message 
			AS ms,guwen_comment AS cmt 
			WHERE blg.ques_id = ms.msgid AND  us.user_id = ms.user_id 
	...
	{
		$sql = "SELECT COUNT(cmt.id) AS num FROM guwen_message AS ms,guwen_comment AS cmt 
			WHERE ms.msgid = cmt.comment_quesid AND  cmt.is_check = 0 
			AND ms.user_id != cmt.comment_uid AND   ms.user_id= ? ";
	...
	public function get_reply_num($uid)
	{
		$sql = "SELECT COUNT(rpy.id) AS num FROM guwen_comment AS cmt, guwen_comment_reply AS rpy , 
			guwen_user AS us ,guwen_message AS ms 
			WHERE cmt.id = rpy.comment_id AND us.user_id = rpy.reply_uid 
			AND cmt.comment_quesid = ms.msgid AND rpy.is_check = 0  AND rpy.reply_uid != cmt.comment_uid
			AND cmt.comment_uid = ?";
		$query = $this->db->query($sql,array($uid));
		$res = $query->result_array();
		return $res[0]['num'];	
	...
	public function get_favour_num($uid)
	{
		$sql = "SELECT  count(slg.id) AS num FROM guwen_user AS us,guwen_message AS ms ,
			guwen_comment as cmt,guwen_snslog AS slg WHERE slg.uid = us.user_id AND slg.is_favour = 0
			AND ms.msgid = slg.quesid  AND slg.comment_id = cmt.id AND slg.is_check = 0 AND 
			slg.uid != cmt.comment_uid AND  cmt.comment_uid = ?";
		$query = $this->db->query($sql,array($uid));
		$res = $query->result_array();
		return $res[0]['num'];	
	}
	...
	{
		$sql = "SELECT COUNT(id) AS num FROM guwen_bestanwserlog 
			WHERE is_check = 0 AND  uid = ?";
		$query = $this->db->query($sql,array($uid));
		$res = $query->result_array();
		return $res[0]['num'];
	}
