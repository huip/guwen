		...
		<div class="my-reply">
			<?foreach ($favour as $key => $value): {?>
				<div class="comment-list-info span11">
					...
					<p class="ft15">中赞了你的回答:<?=$value['comment_content']?></p>
					<p class="sns-time-list"><?=$value["favour_time"]?></p>
				</div>
			<?}endforeach?>
		</div>
		<?if(count($favour) > 0){
                ...
                <h5>热门分类</h5>
                  <?foreach ($hot_cate as $key => $value): {?>
                      <div class="hot-cate-list">
                          <p>
                              <span class="pull-left"><a href="<?=base_url()?>index.php/topic/info/<?=$value['id']?>"><img height="40" width="40" src="<?=$value['tag_img']?>" alt="tag_img" /></a></span>
                              <p>
